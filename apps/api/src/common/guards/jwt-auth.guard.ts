...
/**
 * JwtAuthGuard — Updated version
 *
 * Pehle wala: Khud JWT verify karta tha (manual implementation)
 * Ab: AuthGuard('jwt') extend karta hai — Passport ki JwtStrategy use karta hai
 *
 * Fayde:
 * - JwtStrategy.validate() automatically call hota hai
 * - request.user automatically set hota hai
 * - Standard Passport ecosystem ke saath compatible
 * - Testing mein mock karna easy hai
 *
 * @Public() support:
 * - Agar route pe @Public() laga hai toh guard skip ho jaata hai
 * - Health check, login, register — sab @Public() se exempt hain
 */
@Injectable()
export class JwtAuthGuard extends AuthGuard('jwt') {
  ...
  canActivate(context: ExecutionContext) {
    // @Public() check — pehle yahi check karo
    const isPublic = this.reflector.getAllAndOverride<boolean>(IS_PUBLIC_KEY, [
      context.getHandler(),
      context.getClass(),
    ]);

    // Public route hai toh Passport guard bypass karo
    if (isPublic) return true;

    // Otherwise Passport ka AuthGuard('jwt') kaam kare
    // Ye JwtStrategy.validate() call karega automatically
